...
由于$v_{\pi}(s')$未知，且这是仅对一个状态$s$的估计，因此对算法进行修改：
>[!info] RM算法到TD方法
> 将对状态$s$的**重复采样**$\{(s,r,s')\}$替换为一个trajectory$\{(s_{t},r_{t+1},s_{t+1})\}$，$t$时仅更新被访问到的状态$s_{t}$的状态值估计$v_{t}(s_{t})$，对其他$s \not = s_{t}$则令$v_{t}(s)$不变
