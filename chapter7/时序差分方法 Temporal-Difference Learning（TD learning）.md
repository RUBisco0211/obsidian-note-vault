...
$\delta_{t} = v(s_{t}) - [r_{t+1} + \gamma v(s_{t+1})]= v(s_{t}) - \bar{v}_{t}$ 是两个时刻之间的时序差分（temporal difference），$v(s_{t})$ 是 $t$ 时刻访问的状态 $s_{t}$ 的状态值，$\bar{v}_{t}$ 是算法希望 $v(s_{t})$ 在 $t+1$ 时刻接近的值，**表征了 $v_{t}(s)$ 和 $v_{\pi}(s)$ 之间的误差**
