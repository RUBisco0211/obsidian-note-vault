...
使用[[7.时序差分方法 Temporal-Difference Learning（TD learning）|时序差分方法]]估计$q_{t}(s_{t},a_{t})$即为Actor-Critic方法
## 基本概念
...
其中$w$为Critic函数的参数
- Critic：通过[[8.值函数近似 Value Function Approximation (VFA)#使用VFA的Sarsa|Sarsa]]方法结合值函数估计，输入五元组$(s_{t},a_{t},r_{t+1},s_{t+1},a_{t+1})$，输出对动作值的估计$q_{t}(s_{t},a_{t},w_{t+1})$
...
\end{align*}
$$
步长系数$\delta_{\pi}(s_{t},a_{t}) / \pi(a_{t} | s_{t}, \theta_{t})$也是一个可以平衡exploitation和exploration的量，参考[[9.策略梯度方法 Policy Gradient Methods#^NewStepSize]]
