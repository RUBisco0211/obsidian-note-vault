---
title: 7.时序差分方法 Temporal-Difference Learning（TD learning）
created: 2025-07-05 09:15:27
updated: 2025-11-29 10:48:34
---
## 引入
...
2. 使用[[6.随机近似理论 Stochasted Approximation Theory#Robins-Monro (RM) 算法|RM算法]]求解：$w_{k+1} = w_{k} - \alpha_{k} \tilde{g}(w_{k},\eta_{k}) = w_{k} - \alpha_{k}[w_{k} - (r_{k} + \gamma v(x_{k}))]$

令其中 $R$ 为[[计算机视觉/基本概念#奖励 Reward|奖励]]，$\gamma$ 为[[计算机视觉/基本概念#折扣率 Discount Rate|折扣率]]
...
> [!info] 证明：[[3.贝尔曼最优公式 Bellman Optimality Equation（BOE）#^ContractionMapping|压缩映射]]
...
$$
\mathbb{E}[\delta_{\pi,t} | S_{t} = s_{t}] = v_{\pi}(s_{t}) - \mathbb{E}[R_{t+1} + \gamma v_{\pi}(S_{t+1}) | S_{t} = s_{t}] = 0
$$

其中 $\mathbb{E}[R_{t+1} + \gamma v_{\pi}(S_{t+1}) | S_{t} = s_{t}] = v_{\pi}(s_{t})$ （[[2.贝尔曼公式 Bellman Equation#^bellmanequation|贝尔曼公式]]）

故 $v_{t} = v_{\pi}$ 时 $\delta_{t} = 0$，表征了 $v_{t}$ 和 $v_{\pi}$ 之间的误差

### TD算法的实质：用RM算法求解贝尔曼公式

给出[[2.贝尔曼公式 Bellman Equation#贝尔曼公式 Bellman Equation|贝尔曼公式]]的一个新定义
...
> - Q-Learning 要解决的问题是求解一个贝尔曼最优公式，本身不依赖于任何策略
> $$
> q(s, a) = \mathbb{E} [ R_{t+1} + \gamma \max_{a} q(S_{t+1}, a) \mid S_t = s, A_t = a ], \quad \forall s, a.
> $$
