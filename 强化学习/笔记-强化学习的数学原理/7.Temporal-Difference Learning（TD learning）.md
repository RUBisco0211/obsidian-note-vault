---
title: 7.Temporal-Difference Learning（TD learning）
created: 2025-07-05 09:15:27
updated: 2026-01-16 11:33:05
aliases:
  - 时序差分方法
---
## 引入
...
2. 使用[[6.Stochasted Approximation Theory#Robins-Monro (RM) 算法|RM算法]]求解：$w_{k+1} = w_{k} - \alpha_{k} \tilde{g}(w_{k},\eta_{k}) = w_{k} - \alpha_{k}[w_{k} - (r_{k} + \gamma v(x_{k}))]$
...
> [!info] 证明：[[3.Bellman Optimality Equation（BOE）#^ContractionMapping|压缩映射]]
...
其中 $\mathbb{E}[R_{t+1} + \gamma v_{\pi}(S_{t+1}) | S_{t} = s_{t}] = v_{\pi}(s_{t})$ （[[2.Bellman Equation#^bellmanequation|贝尔曼公式]]）

故 $v_{t} = v_{\pi}$ 时 $\delta_{t} = 0$，表征了 $v_{t}$ 和 $v_{\pi}$ 之间的误差

### TD算法的实质：用RM算法求解贝尔曼公式

给出[[2.Bellman Equation#贝尔曼公式 Bellman Equation|贝尔曼公式]]的一个新定义
