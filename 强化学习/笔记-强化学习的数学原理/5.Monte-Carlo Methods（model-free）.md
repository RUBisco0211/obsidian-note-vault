---
title: 5.Monte-Carlo Methods（model-free）
created: 2025-07-05 09:10:08
updated: 2025-11-29 11:27:08
aliases:
  - 蒙特卡洛方法
---
## 概述
...
基本思想：将[[4.model-based methods#策略迭代 policy iteration (PI)|策略迭代]]中依赖模型，即计算动作值$q(s,a)$的部分进行替换
...
> 其中策略提升的分量形式：
> $$
>  \begin{align*}\pi_{k+1} &=  \arg \max_{\pi} \sum_{a} \pi(a \mid s) \left[ \sum_{r}p(r | s,a)r + \gamma \sum_{s'} p(s' | s,a) v_{\pi_{k}}(s') \right] \\&= \arg \max_{\pi} \sum_{a} \pi(a \mid s) q_{\pi_{k}}(s,a), s \in \mathcal{S}\end{align*} 
> $$
...
$$
- **不依赖模型：[[2.Bellman Equation#动作值 Action value#定义|动作值的原始定义]]**
$$
...
- 多次采样得到采样集 $\{ g^{(j)}(s,a)\}$，则有
$$
q_{\pi_{k}}(s, a) = \mathbb{E}[G_t|S_t = s, A_t = a] \approx \frac{1}{N} \sum_{i=1}^N g^{(i)}(s,a)  
$$
